...
def fftd(img, backwards=False):
    # img的形状可以是(m,n), (m,n,1) 或 (m,n,2)
    # 在我的测试中，numpy和scipy提供的fft比cv2.dft慢
    return cv2.dft(
        np.float32(img),
        flags=(
...
def subwindow(img, window, borderType=cv2.BORDER_CONSTANT):
    ...
    limit(cutWindow, [0, 0, img.shape[1], img.shape[0]])  # 修改cutWindow
...
class KCFTracker:
    def __init__(self, hog=False, fixed_window=True, multiscale=False):
        ...
        if hog:  # HOG特征
            # VOT
            self.interp_factor = 0.012  # 用于自适应的线性插值因子
            self.sigma = 0.6  # 高斯核带宽
            # TPAMI   #interp_factor = 0.02   #sigma = 0.5
            self.cell_size = 4  # HOG单元格大小
            self._hogfeatures = True
        else:  # 原始灰度图像 # 又称CSK跟踪器
            self.interp_factor = 0.055
            self.sigma = 0.5
            self.cell_size = 1
        ...
        if multiscale:
            ...
            self.scale_step = 1.05  # 多尺度估计的比例步长
            self.scale_weight = 0.96  # 为增加稳定性降低其他尺度检测分数的权重
        elif fixed_window:
        ...
        self._tmpl_sz = [0, 0]  # cv::Size, [宽度,高度]  #[int,int]
        self._roi = [
            0.0,
            0.0,
            0.0,
            0.0,
        ]  # cv::Rect2f, [x,y,宽度,高度]  #[float,float,float,float]
        ...
        self._tmpl = None  # numpy.ndarray    原始: (size_patch[0], size_patch[1])   hog: (size_patch[2], size_patch[0]*size_patch[1])
        self.hann = None  # numpy.ndarray    原始: (size_patch[0], size_patch[1])   hog: (size_patch[2], size_patch[0]*size_patch[1])
    ...
    def gaussianCorrelation(self, x1, x2):
        if self._hogfeatures:
            ...
            c = cv2.mulSpectrums(fftd(x1), fftd(x2), 0, conjB=True)  # 'conjB='是必需的!
            c = fftd(c, True)
            c = real(c)
            c = rearrange(c)
    ...
    def getFeatures(self, image, inithann, scale_adjust=1):
        ...
        if inithann:
            self.createHanningMats()  # createHanningMats需要size_patch
    ...
    def detect(self, z, x):
        ...
        _, pv, _, pi = cv2.minMaxLoc(res)  # pv:float  pi:整数元组
