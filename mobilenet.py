...
from tensorflow.keras.layers import (
    Input,
    Convolution2D,  # Consider replacing with Conv2D if using TensorFlow 2.x features
...
def MobileNet_MIX(input_shape=None, num_classes=7):
    ...
    X4 = Lambda(lambda X: tf.compat.v1.image.resize_images(X, size=(n, n)))(x4)
    # X6 = Lambda(lambda X: tf.image.resize_images(X, size=(n, n)))(x6)
    X7 = Lambda(lambda X: tf.compat.v1.image.resize_images(X, size=(n, n)))(x7)
...
if __name__ == "__main__":
    ...
    plot_model(
        model, "models/MobileNet_MIX.png", show_shapes=True, show_layer_names=True
    )  # 保存模型图
    model.summary()
